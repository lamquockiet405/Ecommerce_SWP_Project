...
public class ProductVariantProcess extends DAO.DAO {
    ...
    private boolean updateProductVariant(List<ProductVariant> productVariants) throws SQLException {
        ...
        try (PreparedStatement ps = this.conn.prepareStatement(sql)) {
            for (ProductVariant variant : productVariants) {
                index = 1;
                ps.setString(index++, variant.getProductVariantName() != null ? variant.getProductVariantName() : "");
                ps.setFloat(index++, variant.getPrice());
                ps.setInt(index++, variant.getStockQuantity());
                ps.setString(index++, variant.getImage() != null ? variant.getImage() : "");
                ps.setInt(index++, variant.getStatus());
                ps.setInt(index++, variant.getProductVariantId());
    ...
    private boolean addProductVariant(List<ProductVariant> productVariantList) throws SQLException {
        ...
        try (PreparedStatement ps = this.conn.prepareStatement(sql)) {
            for (ProductVariant pv : productVariantList) {
                index = 1;
                ps.setString(index++, pv.getProductVariantName());
                ps.setFloat(index++, pv.getPrice());
                ps.setInt(index++, pv.getStockQuantity());
                ps.setString(index++, pv.getImage());
                ps.setInt(index++, pv.getProductId());
                ps.setInt(index++, pv.getStatus());
                ps.addBatch();
            }
            index = 1;
            int[] rowsAffected = ps.executeBatch();
            ...
            for (int rows : rowsAffected) {
                if (rows < 0) {
                    this.status = "Batch execution failed for some records";
                    check = false;
                    break;
                }
            }
    ...
    private boolean addProductVariantTransection(List<ProductVariant> productVariantList) throws SQLException {
        try {
            this.conn.setAutoCommit(false);
            if (!addProductVariant(productVariantList)) {
                this.conn.rollback();
                return false;
            }
            this.conn.commit();
