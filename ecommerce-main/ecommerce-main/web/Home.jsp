...
<html lang="en">
                    ...
                    <div class="category-carousel swiper">
                        <div class="swiper-wrapper">
                            <c:forEach var="ct" items="${requestScope.ListCategories}">
                                <a href="productList?&categoryID=${ct.categoryID}" class="nav-link category-item swiper-slide">
                ...
                <div class="col-md-12">
                    ...
                    <div class="bootstrap-tabs product-tabs">
                        <div class="tabs-header d-flex justify-content-between border-bottom my-5">
                            <h3>Products Random</h3>
                            <nav>
                                <div class="nav nav-tabs" id="nav-tab" role="tablist">
                                    <a  class="nav-link text-uppercase fs-6 active" id="nav-all-tab" data-bs-toggle="tab" data-bs-target="#nav-all">All</a>
                                </div>
                            </nav>
                        </div>
                                    ...
                                    <c:forEach var="product" items="${requestScope.MenuChose}">
                                        <div class="col">
                                            <div class="product-item">
                                                ...
                                                <h5 style="font-size: 10px; word-wrap: break-word; overflow-wrap: break-word; white-space: normal;">${product.productName}</h5>
                                                <c:forEach var="price" items="${requestScope.Listprice}">
                                                    <c:if test="${price.productID == product.productID}">
                                                        <span id="priceValue" class="price" data-price="${price.priceMin}">$${price.priceMin}</span>
                                                    </c:if>
                                                </c:forEach>
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function () {
            function formatPrice(number) {
                return number.toLocaleString("en-US"); // ??nh d?ng chu?n d?u ph?y m?i 3 s?
            }
            ...
            function animatePrice(element, value) {
                ...
                let interval = setInterval(() => {
                    ...
                    element.innerText = "$" + formatPrice(Math.floor(start)); // ??nh d?ng s?, làm tròn xu?ng ?? tránh s? l?
                }, 10);
            }

            // L?p qua t?t c? các ph?n t? có class "price"
            document.querySelectorAll(".price").forEach(priceElement => {
                let priceMin = parseFloat(priceElement.dataset.price); // L?y giá t? data-price
                console.log("PriceMin:", priceMin); // Debug ?? ki?m tra giá
    ...
    <style>
        .price {
            ...
            font-size: 24px;
            font-weight: bold;
            color: red !important;  /* ??t !important ?? ch?c ch?n áp d?ng */
    ...
    <style>
        .product-item {
            min-height: 250px; /* ??m b?o t?t c? kh?i s?n ph?m có chi?u cao ??ng ??u */
            display: flex;
        ...
        .product-item img {
            width: 100%;
            height: 150px; /* C? ??nh chi?u cao ?nh */
            object-fit: cover; /* Gi? t? l? ?nh, tránh méo */
        }

        h5 {
            display: -webkit-box;
            -webkit-line-clamp: 2; /* Hi?n th? t?i ?a 2 dòng */
            -webkit-box-orient: vertical;
            overflow: hidden;
            height: 40px; /* Gi? chi?u cao c? ??nh */
        }

        .product-grid.row {
            align-items: stretch; /* Giúp các c?t có chi?u cao b?ng nhau */
        }
