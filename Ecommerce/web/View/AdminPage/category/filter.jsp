...
<div>
    ...
    <form action="${url}" method="get" >
        <div class="d-flex justify-content-between">
            <div id="ctype" class="product-type-container mt-3">
                <a href="${url}?ctype=all&sort=${sort}&search=${search}" class="product_type">All</a>
                <a href="${url}?ctype=live&sort=${sort}&search=${search}" class="product_type">Active</a> 
                <a href="${url}?ctype=unlisted&sort=${sort}&search=${search}" class="product_type">Inactive</a>
            </div>
...
<script>
    function updateProductType() {
        ...
        const ctypeLinks = document.querySelectorAll("#ctype a");

        ctypeLinks.forEach(link => {
            link.classList.remove("active_ptype");
        });

        if (currentLocation.includes("ctype=all")) {
            ctypeLinks[0].classList.add("active_ptype");
        } else if (currentLocation.includes("ctype=live")) {
            ctypeLinks[1].classList.add("active_ptype");
        } else if (currentLocation.includes("ctype=unlisted")) {
            ctypeLinks[2].classList.add("active_ptype");
        } else {
            ctypeLinks[0].classList.add("active_ptype");
        }
    }
